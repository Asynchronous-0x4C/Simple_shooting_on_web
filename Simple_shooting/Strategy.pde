        ...
        new Button(100,120+i*60,250,30).setEvent(new ButtonEvent(){
          void select(Button b){
            ...
            for(int m_index=0;m_index<missions.size();m_index++){
              ...
              boolean clear=false;
              if(saveData.getJSONArray("mission").size()>=stageNumber){
                clear=saveData.getJSONArray("mission").getJSONObject(stageNumber-1).getBoolean(o.getString("attribute"),false);
              }
